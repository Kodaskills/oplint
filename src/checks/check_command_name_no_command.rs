...
/// CMD005: Flag any command name that contains the word "command" (case‑insensitive).
pub fn check(path: &Path, content: &str, ctx: &CheckContext<'_>) -> Vec<Violation> {
    ...
    let Some(name_idx) = super::capture_index(&query, "cmd_name") else {
        return Vec::new();
    };
    ...
    while let Some(m) = matches.next() {
        for capture in m.captures.iter().filter(|c| c.index == name_idx) {
            ...
            if let Ok(cmd_name) = node.utf8_text(content.as_bytes()) {
                if cmd_name.to_lowercase().contains("command") {
