...
/// API005: Flag `MarkdownRenderer.render(content, this, ...)`.
///
/// This directly matches the documented anti‑pattern and is exact for the
/// common case where the class is imported with its original name.
pub fn check(path: &Path, content: &str, ctx: &CheckContext<'_>) -> Vec<Violation> {
    ...
    let Some(ref_idx) = super::capture_index(&query, "plugin_ref") else {
        return Vec::new();
    };
